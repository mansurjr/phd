...
export function NameModal() {
  ...
  const checkExistingUser = async (nameToCheck: string) => {
    try {
      const res = await fetch(`https://api.mutallib.uz/progress/check/${encodeURIComponent(nameToCheck)}`);
  ...
  const handleContinueExisting = () => {
    (async () => {
      ...
      if (!found) {
        ...
        try {
          await fetch(`https://api.mutallib.uz/progress/reset/${existingUser.id}`, { method: 'POST' });
        } catch (e) {
  ...
  const handleStartFresh = async () => {
    if (existingUser) {
      ...
      try {
        // Reset progress
        await fetch(`https://api.mutallib.uz/progress/reset/${existingUser.id}`, {
          method: 'POST',
        });
